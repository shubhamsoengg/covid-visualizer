...
function loadMapFunc(geoData, geoKeys, delayTime) {
    ...
    async function drawRegionsMap() {
        ...
        for (let i = 0; i < geoKeys.length; i++) {
            chart.draw(google.visualization.arrayToDataTable(geoData[geoKeys[i]]), options);
...
function loadChartFunc(pageLoad, rawData, lineChartKeys, delayTime) {
    ...
    async function drawChart() {
        ...
        if (pageLoad) {
            ...
            document.getElementsByClassName("total-cases-wrapper")[0].style.display = "inline-block";
            document.getElementsByClassName("active-cases-wrapper")[0].style.display = "inline-block";
            for (let i = 0; i < lineChartKeys.length; i++) {
                ...
                if (lineChartKeys[i] <= currentDate) {
                    ...
                    if (total <= 10000) {
                        ...
                        caseHTML.innerHTML = total + " <i class = 'fa fa-exclamation-triangle '></i>";
                    } else if (total > 320000) {
                        caseHTML.style.color = "#F44336";
                        caseHTML.innerHTML = total + " <i class = 'fa fa-exclamation-triangle '></i>";
                    }
                } else {
                    document.getElementById("prediction-span").style.display = "inline-block";
                }
